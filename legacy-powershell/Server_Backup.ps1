    ...
    continue
        ...
        if ($body.sourcePackageId -and $body.targetPackageId) {
          ...
          foreach ($artifact in $sourceArtifacts) {
            ...
            if (-not $global:cachedArtifactStatus.ContainsKey($artifactKey)) {
              ...
              Write-Host "📡 Using Cached Deployment Status for Source Artifact : $($artifact.Id)"
              $cachedStatus = $global:cachedArtifactStatus[$artifactKey]
              $artifact.Status = $cachedStatus.Status
              $artifact.DeployedOn = $cachedStatus.DeployedOn
            }
          }
        ...
        elseif ($body.sourceArtifactId -and $body.targetArtifactId) {
          ...
          if (-not $global:cachedManifests.ContainsKey($sourceArtifactKey)) {
            ...
            Write-Host "📜 Using Cached Manifest for Source Artifact..."
            $sourceManifest = $global:cachedManifests[$sourceArtifactKey]
          }

          # ** Fetch & Cache Manifest Data for Target Artifact **
          if (-not $global:cachedManifests.ContainsKey($targetArtifactKey)) {
            ...
            Write-Host "📜 Using Cached Manifest for Target Artifact..."
            $targetManifest = $global:cachedManifests[$targetArtifactKey]
          }

          # ** Fetch & Cache Configurations for Source Artifact **
          if (-not $global:cachedConfigurations.ContainsKey($sourceArtifactKey)) {
            ...
            Write-Host "⚙️ Using Cached Configurations for Source Artifact..."
            $sourceConfigurations = $global:cachedConfigurations[$sourceArtifactKey]
          }

          # ** Fetch & Cache Configurations for Target Artifact **
          if (-not $global:cachedConfigurations.ContainsKey($targetArtifactKey)) {
            ...
            Write-Host "⚙️ Using Cached Configurations for Target Artifact..."
            $targetConfigurations = $global:cachedConfigurations[$targetArtifactKey]
          }
          # ** Ensure Response is in JSON Format **
          $response.ContentType = "application/json"
